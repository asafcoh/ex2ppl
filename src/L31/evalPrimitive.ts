...
const dictPrim = (args: Value[]): Result<Value> =>
    args.length === 1 ? makeOk(args[0]) : 
    makeFailure("dict expects exactly one argument");

const getPrim = (args: Value[]): Result<Value> => {
        if (args.length !== 2) return makeFailure("get expects exactly 2 arguments");
    
        const [dict, key] = args;
    
        if (!isCompoundSExp(dict)) return makeFailure("First argument to get must be a dict (compound S-exp)");
        if (!isSymbolSExp(key)) return makeFailure("Second argument to get must be a symbol");
    
        // נלך איבר אחר איבר בתוך רשימת ה-compound
        ...
        while (isCompoundSExp(current)) {
            ...
            current = current.val2; // קפיצה קדימה ברשימה
        }
    
        return makeFailure(`Key '${key.val}' not found in dict`);
    };

const isDictPrim = (args: Value[]): Result<Value> => {
        if (args.length !== 1) {
        ...
        // נעבור על הרשימה — נוודא שכל איבר הוא זוג (CompoundSExp)
        let current: Value = dict;
    
        while (isCompoundSExp(current)) {
            const pair = current.val1;
    
            // כל איבר ברשימה חייב להיות גם הוא זוג (a . 1)
        ...
        // אחרי הלולאה – אם סיימנו בלי בעיה, זו באמת רשימת זוגות
        return makeOk(true);
    };
